...
%		
		...
		\item[Answer:] Given a linear operator $L  : {\mathbb{C}}^{n} \rightarrow {\mathbb{C}}^{n}$ defined by $L(x) = Ax$ with the natural basis ($u = \{{u}^{1} \ldots {u}^{n}\}$) applied on both, the domain and codomain.
			...
			\end{equation}
			We know that for the natural basis $u$, % = \begin{bmatrix}1 & 0 & 0 & \ldots & 0 \\ 0 & 1 & 0 & \ldots & 0 \\ 0 & 0 & 1 & \ldots & 0 \\ \vdots & \vdots & \vdots & \vdots & \vdots \\ 0 & 0 & 0 & \ldots & 1\end{bmatrix} 
			$A \times u = A$.
			\begin{equation}
				\hat{A}{[x]}_{u} = A {[x]}_{u}
			\end{equation}
		...
		\item[Answer:] Given a linear operator $L  : {\mathbb{C}}^{n} \rightarrow {\mathbb{C}}^{n}$ defined by $L(x) = Ax$ with the basis created of eigenvectors of A ($M = \{ {v}^{1}, {v}^{2}, \ldots {v}^{n}\}$) applied on the domain.
			...
			\begin{equation}
				{[\Lambda]}_{u} = \begin{bmatrix} {\lambda}_{1} \\ {\lambda}_{2} \\ \vdots \\ {\lambda}_{n}\end{bmatrix} \in {\mathbb{C}}^{n}
			\end{equation}
			
			Now let us compute the matrix reperesntation, 
			\begin{equation}
			{[L(\Lambda)]}_{M} = {[L( {\lambda}_{1}{v}^{1} + \ldots + {\lambda}_{n}{v}^{n})]}_{M}
			\end{equation}
			\begin{equation}
				{[L(\Lambda)]}_{M} = {[L({\lambda}_{1}{v}^{1}) + \ldots + L({\lambda}_{n}{v}^{n})]}_{M}
			\end{equation}
			\begin{equation}
				{[L(\Lambda)]}_{M} = {[{\lambda}_{1}L({v}^{1}) + \ldots + {\lambda}_{n}L({v}^{n})]}_{M}
			\end{equation}
%			From the theorem on matrix representation, we know that ${[L(x)]}_{v} = A{[x]}_{u}$. \\ So, above equation can be rewitten as, 
%			\begin{equation}
%				\hat{A}{[\Lambda]}_{M} = {[{\lambda}_{1}L({v}^{1}) + \ldots + {\lambda}_{n}L({v}^{n})]}_{M}
%			\end{equation}
			\begin{equation}
				{[A\Lambda]}_{M} = {[{\lambda}_{1}A{v}^{1} + \ldots + {\lambda}_{n}A{v}^{n}]}_{M}
			\end{equation}
			\begin{equation}
				{[A\Lambda]}_{M} = {[{\lambda}_{1}A{v}^{1}]}_{M} + \ldots + {[{\lambda}_{n}A{v}^{n}]}_{M}
			\end{equation}
