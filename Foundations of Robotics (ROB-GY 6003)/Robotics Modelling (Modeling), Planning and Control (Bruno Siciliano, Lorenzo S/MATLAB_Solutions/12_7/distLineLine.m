...
% Here we use m and n for respectively for gamma and delta

% L. Villani, G. Oriolo, B. Siciliano
% February 2009

if ~( strncmp(b1.type, 'line', 4) && strncmp(b2.type, 'line', 4) )
    error('inputs are not line segments')
end
...
if lP && lQ %if both segments have non zero length
    ...
    if rank([eP',-eQ'])==2  %if segments are not parallel
